...
update_all_existing_agents() {
    ...
    # Track processed file paths to avoid duplicate updates
    declare -A processed_files
    
    # Check each possible agent file and update if it exists (only once per file path)
    ...
    if [[ -f "$QODER_FILE" ]] && [[ -z "${processed_files[$QODER_FILE]:-}" ]]; then
        update_agent_file "$QODER_FILE" "Qoder CLI"
        processed_files["$QODER_FILE"]=1
        found_agent=true
    fi
